...
O relatório descreve o desenvolvimento de um compilador capaz de traduzir programas escritos em Fortran 77 para código executável na EWVM (EPL Web Virtual Machine), uma máquina virtual desenvolvida no Departamento de Informática da Universidade do Minho.
...
O compilador está organizado em 7 módulos Python: 6 correspondentes às fases da pipeline de compilação e a `symbol_table`, que é partilhada entre a análise semântica e a geração de código:

Ficheiro .f → preprocess() → lexer.py → parser.py → semantic.py → optimizer.py → codegen.py → Código .vm
...
- **Palavras reservadas** (19): PROGRAM, END, INTEGER, REAL, LOGICAL, IF, THEN, ELSE, ENDIF, DO, CONTINUE, GOTO, PRINT, READ, STOP, RETURN, CALL, FUNCTION, SUBROUTINE (nota: o token SUBROUTINE é reconhecido pelo lexer mas não foi implementado no parser nem no gerador de código; apenas FUNCTION foi desenvolvido como valorização)
...
O Fortran 77 é case-insensitive, pelo que a função `t_ID` converte todos os identificadores para maiúsculas antes de verificar se correspondem a uma palavra reservada. As keywords são capturadas pela expressão regular do identificador e depois reclassificadas via dicionário:
...
A AST é representada como tuplos Python aninhados. Cada nó é um tuplo cujo primeiro elemento identifica o tipo. Exemplos:
...
A EWVM é uma máquina virtual de stack. O código gerado utiliza a região global da stack para armazenar variáveis, e a stack de operandos para avaliar expressões (travessia post-order da AST). Os principais padrões de geração são:
...
Implementámos suporte a `INTEGER FUNCTION` como valorização. Como a EWVM não tem stack frames, optámos por passar os argumentos via variáveis globais.
...
Esta abordagem é simples e funciona bem para o nosso caso, embora não permita recursão.
...
Foram implementadas duas valorizações: suporte a funções com `FUNCTION/CALL/RETURN` e otimização por Constant Folding. A análise semântica inclui ainda verificação rigorosa de tipos.
